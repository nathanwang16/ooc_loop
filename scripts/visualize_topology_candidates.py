...
def _new_axes(title: str, xlim, ylim, *, height=5.2):
    fig, ax = plt.subplots(figsize=(13.0, height))
    ax.set_title(title, fontsize=17, pad=12)
    ax.set_xlabel("x  (flow direction, μm)", fontsize=14, labelpad=8)
    ax.set_ylabel("y  (μm)", fontsize=14, labelpad=12)
...
def _add_colorbar(fig, ax):
    ...
    cb.set_label("inlet concentration  C", fontsize=14)
...
def render_christmas_tree():
    ...
    fig, ax = _new_axes("B. Christmas-tree pre-mixer + chamber  →  axis=y linear gradient",
                        xlim=(tree_x_start - 1900, L + 1900), ylim=(-W * 0.30, W * 1.20),
                        height=6.0)
    _draw_chamber(ax)
    ...
    ax.text(tree_x_start - 400, -W * 0.22, "2 reagent inputs", fontsize=13, color="#374151")
    ax.text((tree_x_start + tree_x_end) / 2, W * 1.10,
            "binary serpentine mixer tree (2 → 3 → 5 → 8 streams)",
            fontsize=13, color="#374151", ha="center")
...
def render_side_injection(K: int = 8):
    fig, ax = _new_axes(f"C. Distributed side-injection  (K={K} drug ports along y=0)  →  axis=x linear gradient",
                        xlim=(-2700, L + 1700), ylim=(-W * 0.65, W + 500))
    _draw_chamber(ax)
    ...
    ax.text(-550, W / 2, "medium\nC=0", ha="right", va="center", fontsize=14,
    ...
    for k in range(K):
        ...
        q_frac = (k + 1) / K
        ax.text(xc, -port_h - 280, f"Q$_{{{k+1}}}$={q_frac:.2f}",
                ha="center", fontsize=11, color="#7f1d1d")

    ax.text(L / 2, -W * 0.58, "drug ports along y=0 wall, increasing $Q_k \\propto k$",
            fontsize=13, color="#7f1d1d", ha="center")
    ...
    ax.annotate("", xy=(L * 0.55, W * 0.85), xytext=(L * 0.10, W * 0.85),
                arrowprops=dict(arrowstyle="->", color="#374151", lw=2.0))
    ax.text(L * 0.32, W * 0.93, "advection (+x)", fontsize=14, color="#374151")
...
def render_permeable_wall():
    fig, ax = _new_axes("D. Permeable floor membrane + reservoir  →  axis=x linear gradient",
                        xlim=(-2700, L + 1700), ylim=(-450, W + 900))
    _draw_chamber(ax)
...
def render_counter_flow():
    fig, ax = _new_axes("E. Counter-flow inlets  →  axis=x  (REJECTED — unsteady-flow risk)",
                        xlim=(-3000, L + 3000), ylim=(-W * 0.3, W * 1.65))
    _draw_chamber(ax)
    ...
    ax.text(-650, W * 0.5, "drug\nC=1\n(low Q)", ha="right", va="center",
            fontsize=13, color="#7f1d1d", fontweight="bold")
    ...
    ax.text(L * 0.5, W + 1000, "opposing flows; stagnation surface mid-chamber",
            fontsize=13, color="#374151", ha="center")
    ax.text(L * 0.5, W + 500, "side outlets at y=0 and y=W (mid-chamber)",
            fontsize=13, color="#15803d", ha="center")

    # Counter-flow arrows
    ax.annotate("", xy=(L * 0.45, W * 0.5), xytext=(L * 0.05, W * 0.5),
                arrowprops=dict(arrowstyle="->", color="#7f1d1d", lw=2.2))
